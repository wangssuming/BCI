...
mkdir(fullfile('C:\Users\ASUS\Desktop\S1_M1_ICMS_4\area_vel_New',Date));
Write = fullfile('C:\Users\ASUS\Desktop\S1_M1_ICMS_4\area_vel_New',Date);
...
saveas(figure(1),fullfile(Write,'LFP_Area_to_Velocity_gain1'))
saveas(figure(2),fullfile(Write,'LFP_TotalArea_to_Velocity_gain1'))
name1 = 'Ar_Vel';
name2 = 'Area_Ve';
...
for ch = 1:8
    ...
    for i = 1:floor(length(Combine_DAQ_plx.Event_touch{1,1})/8)
        for tri = 1:8
            q = 8*(i-1)+tri;
            Trial = sprintf('%s_%d','Trial',q);
            PlotTrial = Area_Ve.(Channel).Origin(q,:);        
            figure(ch+(8*(i-1))+2);
            subplot(2,4,tri)
        ...
        name = sprintf('%s_%d_%d','Channel',ch,i);
        saveas(figure(ch+(8*(i-1))+2),fullfile(Write,name));
    end
       
    if mod(length(Combine_DAQ_plx.Event_touch{1,1}),8) ~= 0
       for tri = 1:mod(length(Combine_DAQ_plx.Event_touch{1,1}),8)
           q = 8*floor(length(Combine_DAQ_plx.Event_touch{1,1})/8)+tri;
           Trial = sprintf('%s_%d','Trial',q);
           PlotTrial = Area_Ve.(Channel).Origin(q,:);        
           figure(ch+(8*floor(length(Combine_DAQ_plx.Event_touch{1,1})/8))+2);
           subplot(2,4,tri)
           plot(timeseries,PlotTrial)
           hold on
           plot(timeseries(max+(pre_time_1 - pre_time_2)*fs),PlotTrial(max+(pre_time_1 - pre_time_2)*fs),'ro')
       ...
       name = sprintf('%s_%d_%d','Channel',ch,floor(length(Combine_DAQ_plx.Event_touch{1,1})/8)+1);
       saveas(figure(ch+(8*floor(length(Combine_DAQ_plx.Event_touch{1,1})/8))+2),fullfile(Write,name));
    end
end